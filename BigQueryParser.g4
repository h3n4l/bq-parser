/*
 * WIP BigQuery grammar for ANTLR v4 This grammar is designed to parse BigQuery SQL statements, and
 * is based off of the BigQuery SQL syntax specified at:
...
expr:
	...
	| array_name LSB (
		...
		| SAFE_OFFSET
		| SAFE_ORDINAL
	) LR_BRACKET expr RR_BRACKET RSB
	| unary_operator expr
...
// column_expr : BACKTICK column_expr BACKTICK | (((project_name DOT)? dataset_name DOT)? table_name
// DOT)? column_name ;

column_expr: name dot_name*;
...
// Name can be any ID or string, with optional quotes and parens name : ID | '"' name '"' |
// LR_BRACKET name RR_BRACKET | BACKTICK name BACKTICK | '\'' name '\'' ;
name: ID | QUOTED_ID;

dot_name: DOT (name | keyword);
